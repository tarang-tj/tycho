"""Per-body site configs and processing steps for the TYCHO terrain
pipeline, split out of build_terrain.py (U4: keep files under ~250 lines).
build_terrain.py's CLI (`main()`) just calls process_moon()/process_mars()/
process_lunokhod() from here; nothing about the CLI contract or the output
file contract changed.
"""
